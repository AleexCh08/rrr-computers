...
export default function AdminReturns() {
  ...
  useEffect(() => {
    const fetchReturns = async () => {
      try {
        const response = await api.get(`devoluciones/?page=${currentPage}&search=${activeSearch}`);
        ...
        <div style={{ background: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ border: '1px solid #eaeaea', borderRadius: '8px', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {returns.length === 0 ? (
                <div style={{ padding: '30px', textAlign: 'center', color: '#666' }}>No hay devoluciones registradas en el sistema.</div>
              ) : (
